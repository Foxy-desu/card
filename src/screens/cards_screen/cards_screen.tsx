...
const CardsScreen = () => {
  ...
  const { ref, currentPage, shouldFetch, setShouldFetch, shouldShowUpdate, setShouldShowUpdate } = useIntersect(
    isLoading,
    popupStats.isOpen,
    error,
  );
  ...
  return (
    <div className={cl.cardScreen} id="cardScreen" ref={cardScreenRef}>
      ...
      <main className={cl.main}>
        ...
        <div className={cl.observer} style={{ height: '2vw' }} ref={ref}></div>
        {popupStats.isOpen && (
          <PopUp
            pType={popupStats.pType}
            error={popupStats.error}
            cardData={popupStats.cardData}
            closePopup={closePopUp}
            referer={cardScreenRef.current}
          />
        )}
      </main>
    </div>
